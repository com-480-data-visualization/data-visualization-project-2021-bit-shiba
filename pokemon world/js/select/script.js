function filterPokemon(option, checked) {
  ...
  var includePkmnFromBothVersions =
    $("#version-filter [value=both]:selected").length > 0;
  // get evolution filters
  var includeNotFullyEvolvedPkmn =
    $("#evolution-filter [value=nfe]:selected").length > 0;
  var includeFullyEvolvedPkmn =
    $("#evolution-filter [value=fe]:selected").length > 0;
  ...
  $("ol.picker li").each(function () {
    ...
    if (
      ...
      // filter by version
      if (isAlolaDex) {
        var includePkmnFromSun =
          $("#version-filter [value=sun]:selected").length > 0;
        var includePkmnFromMoon =
          $("#version-filter [value=moon]:selected").length > 0;
        if (
          ($this.hasClass("sun") && !includePkmnFromSun) ||
          ($this.hasClass("moon") && !includePkmnFromMoon) ||
          (!$this.hasClass("sun") &&
            !$this.hasClass("moon") &&
            !includePkmnFromBothVersions)
        ) {
          return;
        }
      }
      if (isKalosDex) {
        var includePkmnFromX =
          $("#version-filter [value=x]:selected").length > 0;
        var includePkmnFromY =
          $("#version-filter [value=y]:selected").length > 0;
        if (
          ($this.hasClass("x") && !includePkmnFromX) ||
      ...
      if (isHoennDex) {
        var includePkmnFromOmegaRuby =
          $("#version-filter [value=ruby]:selected").length > 0;
        var includePkmnFromAlphaSapphire =
          $("#version-filter [value=sapphire]:selected").length > 0;
        if (
          ($this.hasClass("ruby") && !includePkmnFromOmegaRuby) ||
          ($this.hasClass("sapphire") && !includePkmnFromAlphaSapphire) ||
          (!$this.hasClass("ruby") &&
            !$this.hasClass("sapphire") &&
            !includePkmnFromBothVersions)
        ) {
          return;
        }
      ...
      if (
        !$this.hasClass("normal1") &&
        ($this.children(":first").hasClass("arceus") ||
          $this.children(":first").hasClass("silvally")) &&
        types.length >= 36
      ) {
        return;
      }
...
function addPkmnToTeam($this) {
  ...
  if (
    $this.attr("class").includes("necrozma") &&
    $("ul.picked .art[class*=necrozma]").length > 0
  ) {
  ...
  if (
    $this.attr("class").includes("-mega") &&
    $("ul.picked .art[class*=-mega]").length > 0
  ) {
...
function removePkmnFromTeam($this) {
  ...
  $("ol.picker .menu-sprite." + pkmnClass)
    .parent()
    .removeClass("picked");
  if (pkmnClass == "arceus" || pkmnClass == "silvally") {
    $("ol.picker .menu-sprite." + pkmnClass).each(function () {
...
function updateTeamTypeAnalysis(pkmnName, removed) {
  ...
  $("#team-weaknesses .type").each(function () {
    ...
    if (
      $("ol.picker .picked .menu-sprite." + pkmnName)
        .parent()
        .hasClass("weak2-" + type)
    ) {
      var $td = $this.find("td");
  ...
  $.get("./static/type_data.xml", function (d) {
    ...
    $("ol.picker li").each(function () {
      ...
      if (types.length > 1) {
        ...
        for (type in type_matchup) {
          type_matchup[type] =
            type_matchups[types[0]][type] * type_matchups[types[1]][type];
        }
      } else {
    ...
    if (window.location.hash) {
      window.location.hash
        .substring(1)
        .split("+")
        .forEach(function (pkmnClass) {
          if (
            pkmnClass.startsWith("arceus") ||
            pkmnClass.startsWith("silvally")
          ) {
            ...
            $("ol.picker ." + type + " .menu-sprite." + pkmnClass).trigger(
              "click"
            );
          } else {
  ...
  $("#randomize").click(function (e) {
    ...
    if ($filteredPkmn.length > 0) {
      ...
      var includeFullyEvolvedPkmn =
        $("#evolution-filter [value=fe]:selected").length > 0;
      var includeMegas =
        $("#evolution-filter [value=mega]:selected").length > 0;
      if (
        includeMegas &&
        !includeFullyEvolvedPkmn &&
        !includeNotFullyEvolvedPkmn
      ) {
        teamSize = 1;
      }
